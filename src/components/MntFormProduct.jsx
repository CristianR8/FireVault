...
// Función para realizar el mantenimiento en la seccion de cronograma
const MntFormProduct = ({ onSubmitFn }) => {

  // Metodos implementados del hook de useForm
  const {
    register,
    handleSubmit,
    setError,
  ...
  // Variables necesarias para el codigo y la fecha.
  const codigoValue = getValues('codigo');
  const fechaValue = getValues('fecha')

  // Objetos user y logout necesarios para  la autenticación
  const { user, logout } = useAuth();

  // Variable handleSwal es nula por defecto
  let handleSwal = null

  // Si el codigo y la fecha son validos, se muestra un mensaje de confirmacion.
